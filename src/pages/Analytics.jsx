...
const CATEGORY_COLORS = {
  snacks: "#eab308", repairs: "#ef4444", utilities: "#3b82f6",
  rent: "#a855f7", salaries: "#22c55e", other: "#6b7280",
};
...
export default function Analytics() {
  ...
  useEffect(() => {
    Promise.all([
      base44.entities.Session.list("-created_date", 2000),
  ...
  const monthSessions = sessions.filter(
    (s) => s.status === "completed" && s.start_time?.startsWith(selectedMonth)
  );
  ...
  const trend = Array.from({ length: 12 }, (_, i) => {
    ...
    const earn = sessions
      .filter((s) => s.status === "completed" && s.start_time?.startsWith(key))
  ...
  // Console utilization for selected month
  const consoleMap = {};
  ...
  const consoleUtil = Object.entries(consoleMap)
  ...
  const downloadCSV = () => {
    const rows = [];
    rows.push([`=== ANALYTICS REPORT — ${selectedMonth} ===`]);
    rows.push([]);
    rows.push(["KPI", "Value"]);
    rows.push(["Earnings", monthEarnings.toFixed(2)]);
    rows.push(["Expenses", monthExpTotal.toFixed(2)]);
    rows.push(["Net Profit", netProfit.toFixed(2)]);
    rows.push(["Sessions", monthSessions.length]);
    rows.push([]);
    rows.push(["=== EXPENSES BY CATEGORY ==="]);
    rows.push(["Category", "Amount"]);
    expCategories.forEach(([cat, amt]) => rows.push([cat, amt.toFixed(2)]));
    rows.push([]);
    rows.push(["=== CONSOLE UTILIZATION ==="]);
    rows.push(["Console", "Type", "Sessions", "Hours"]);
    consoleUtil.forEach(([name, d]) => rows.push([name, d.type, d.sessions, (d.minutes / 60).toFixed(1)]));
    rows.push([]);
    rows.push(["=== 12-MONTH TREND ==="]);
    rows.push(["Month", "Earnings", "Expenses", "Profit"]);
    trend.forEach((t) => rows.push([t.month, t.earnings, t.expenses, t.profit]));
    ...
    a.href = url;
    a.download = `gamezone-analytics-${selectedMonth}.csv`;
    a.click();
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-game-muted text-sm mt-1">Monthly business performance</p>
        </div>
        <div className="flex items-center gap-3">
          <select
            value={selectedMonth}
            onChange={(e) => setSelectedMonth(e.target.value)}
            className="bg-game-surface border border-game-border text-white text-sm rounded-lg px-3 py-2"
          >
            {monthOptions.map((m) => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Earnings", value: `$${monthEarnings.toFixed(2)}`, color: "text-green-400", border: "border-green-500/20", icon: DollarSign },
          { label: "Expenses", value: `$${monthExpTotal.toFixed(2)}`, color: "text-red-400", border: "border-red-500/20", icon: Receipt },
          { label: "Net Profit", value: `$${netProfit.toFixed(2)}`, color: netProfit >= 0 ? "text-blue-400" : "text-red-400", border: "border-blue-500/20", icon: TrendingUp },
          { label: "Sessions", value: monthSessions.length, color: "text-purple-400", border: "border-purple-500/20", icon: Monitor },
        ].map(({ label, value, color, border, icon: Icon }) => (
          <div key={label} className={`bg-game-surface border ${border} rounded-xl p-4`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-game-muted text-xs">{label}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-game-surface border border-game-border rounded-xl p-5">
          ...
          {expCategories.length === 0 ? (
            <p className="text-game-muted text-sm">No expenses this month</p>
          ) : (
        ...
        {/* Console utilization */}
        <div className="bg-game-surface border border-game-border rounded-xl p-5">
          <h3 className="text-white font-semibold mb-4">Console Utilization</h3>
          {consoleUtil.length === 0 ? (
            <p className="text-game-muted text-sm">No sessions this month</p>
          ) : (
            <div className="space-y-3">
                ...
                return (
                  <div key={name}>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-game-muted text-sm">{d.sessions} sessions · {(d.minutes / 60).toFixed(1)}h</span>
                    </div>
                    <div className="h-1.5 bg-game-bg rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all"
                        style={{ width: `${pct}%`, backgroundColor: d.type === "PS5" ? "#3b82f6" : "#a855f7" }}
                      />
                    </div>
                  </div>
                );
      ...
      {/* Monthly earnings vs expenses bar chart */}
      <div className="bg-game-surface border border-game-border rounded-xl p-5">
        <h3 className="text-white font-semibold mb-4">Earnings vs Expenses — Last 12 Months</h3>
        <ResponsiveContainer width="100%" height={220}>
