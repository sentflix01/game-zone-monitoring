...
export default function Expenses() {
  ...
  const load = () =>
    base44.entities.Expense.list("-date").then((e) => {
      setExpenses(e);
      setLoading(false);
    });
  ...
  const add = async () => {
    if (!form.amount || isNaN(parseFloat(form.amount))) return toast.error("Enter a valid amount");
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <p className="text-game-muted text-sm mt-1">Track daily shop expenses</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-game-surface border border-red-500/20 rounded-xl p-4">
          <p className="text-game-muted text-xs mb-1">Today's Expenses</p>
        ...
        <div className="bg-game-surface border border-orange-500/20 rounded-xl p-4">
          <p className="text-game-muted text-xs mb-1">This Month</p>
          <p className="text-2xl font-bold text-orange-400">${monthTotal.toFixed(2)}</p>
        </div>
      </div>

      {/* Add form */}
      <div className="bg-game-surface border border-game-border rounded-xl p-5 space-y-4">
        <h3 className="text-white font-semibold flex items-center gap-2">
          <Plus className="w-4 h-4 text-blue-400" /> Add Expense
        </h3>
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button onClick={add} className="bg-blue-600 hover:bg-blue-500 text-white">
            Add
          </Button>
        </div>
      </div>
          ...
          <div key={e.id} className="bg-game-surface border border-game-border rounded-xl px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                variant="outline"
                size="icon"
                className="border-red-500/30 text-red-400 hover:bg-red-600/20 w-7 h-7"
              >
